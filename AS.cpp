...
int erreurFONC(Arg * liste)  // fonction qui detecte si ils manquent des parentheses apres une fonction
{
    ...
    while (compt < taille)
    {
        ...
        {
            p = compt;
            printf ("Erreur : parenthese non placee sur la fonction\n");
            printErreur (liste, p);
            res = 1;
        }
        compt++;
...
int erreur_syntaxe(Arg* tab)
{
    int res=0;
    int i,taille=tailleTableau(tab);
    for(i=0; i<taille; i++)
    {
        if(tab[i].valeur.erreur==NO_TOKEN)
        {
            printf("Erreur: Fonction inexistante");
            res=1;
        }
        if(tab[i].valeur.erreur==TROP_DE_VIRGULES)
        {
...
int erreurOPP(Arg * liste){ // fonction qui detecte si deux opperateurs se suivent
    int compt = 0;
    int taille = tailleTableau (liste)-1;
    int p = 0;
    int res=0;
    ...
    while (compt < taille){
        if (liste[compt].type==OPERATEUR && liste[compt+1].type==OPERATEUR) {
            p = compt;
            printf ("Erreur : deux signes de suite\n");
            printErreur (liste, p);
            res=1;
        }
        compt++;
